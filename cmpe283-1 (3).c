...
*/
struct capability_info procpri[22] =
{
...
*/
struct capability_info procsec[27] =
{
...
exit controls
*/
struct capability_info closesvm[14] =
{
...
struct capability_info virtualentry[12] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("Procbased primary Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procpri, 22, lo, hi);
	
	/* Procbased secondary controls */
	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("Procbased secondary Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procsec, 27, lo, hi);
	
	/* exit controls */
	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
	pr_info("Exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(closesvm, 14, lo, hi);
	
	/* entry controls */
	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("Entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(virtualentry, 12, lo, hi);
